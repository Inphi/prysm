...
func TestServer_buildHeaderBlock(t *testing.T) {
	...
	vs := &Server{StateGen: stategen.New(db, doublylinkedtree.New(nil)), BeaconDB: db}
...
func TestServer_getAndBuildHeaderBlock(t *testing.T) {
	...
	vs.ForkFetcher = &blockchainTest.ChainService{ForkChoiceStore: doublylinkedtree.New(nil)}
	...
	vs.StateGen = stategen.New(vs.BeaconDB, doublylinkedtree.New(nil))
...
func TestServer_GetBellatrixBeaconBlock_BuilderCase(t *testing.T) {
	...
	proposerServer.ForkFetcher = &blockchainTest.ChainService{ForkChoiceStore: doublylinkedtree.New(&dummyDA{})}
	randaoReveal, err := util.RandaoReveal(beaconState, 0, privKeys)
	require.NoError(t, err)
...
func TestServer_circuitBreakBuilder(t *testing.T) {
	...
	s.ForkFetcher = &blockchainTest.ChainService{ForkChoiceStore: doublylinkedtree.New(&dummyDA{})}
