...
func TestStore_OnBlockBatch(t *testing.T) {
	...
	fc := doublylinkedtree.New(&mockDataAvailability{})
	opts := []Option{
		WithDatabase(beaconDB),
...
func TestFillForkChoiceMissingBlocks_CanSave(t *testing.T) {
	...
	opts := []Option{
		WithDatabase(beaconDB),
		WithStateGen(stategen.New(beaconDB, doublylinkedtree.New(&mockDataAvailability{}))),
	}
	service, err := NewService(ctx, opts...)
	require.NoError(t, err)
	...
	tests := []struct {
		...
		},
		{
			...
			header: func() interfaces.ExecutionDataHeader {
				h, err := consensusblocks.NewExecutionDataHeader(&enginev1.ExecutionPayloadHeader{
					ParentHash:       make([]byte, fieldparams.RootLength),
					FeeRecipient:     make([]byte, fieldparams.FeeRecipientLength),
					StateRoot:        make([]byte, fieldparams.RootLength),
					ReceiptsRoot:     make([]byte, fieldparams.RootLength),
...
func TestNoViableHead_Reboot(t *testing.T) {
	...
	require.NoError(t, err)
	newfc := doublylinkedtree.New(&mockDataAvailability{})
	opts := []Option{
		WithDatabase(beaconDB),
		WithAttestationPool(attestations.NewPool()),
