	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				...
				prot, err := h.Proto()
				require.NoError(t, err)
				got, ok := prot.(*enginev1.ExecutionPayloadHeader)
				require.Equal(t, true, ok)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				...
				require.Equal(t, tt.err, err)
				protH, err := tt.header.Proto()
				require.NoError(t, err)
				want, ok := protH.(*enginev1.ExecutionPayloadHeader)
				require.Equal(t, true, ok)
				h, err := st.LatestExecutionPayloadHeader()
				require.NoError(t, err)
				prot, err := h.Proto()
...
func emptyPayloadHeader() (interfaces.ExecutionDataHeader, error) {
	return consensusblocks.NewExecutionDataHeader(&enginev1.ExecutionPayloadHeader{
